...
def gamma_exposure(basecoin: str = 'BTC', plt_save_path: str = 'gamma_exposure.png'):
    ...
    for i in range(-int(spot*0.1), int(spot*0.1), 100):
        ...
        hedge_delta = (
            last_prices.pipe(lambda df: df[df['iv']!=0])
                ...
                .assign(HDELTA = lambda df: df['HDELTA'].mask(df['CP']=='P', -df['HDELTA']))
                .assign(S = _S)
        )
    ...
    # Plot Gamma Exposure
    market_gamma = (
        pd.concat(hedge_delta_list)
            .assign(CP_EXPIRY = lambda df: df['CP'] + '-' + df['EXPIRY'].dt.strftime('%d%b%y') + '-' +df['K'].astype(str))
    ...
    ax.set_ylabel('Gamma Exposure (Dollar/1% Spot Move)')
